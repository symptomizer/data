            ...
            for (let i = 0; i < relatedLink_len; i++) {
              if (relatedLink[i].relatedLink === undefined) {
                ...
                if (relatedLink[i]["url"].toString().charAt(0) === "/") {
                  current_related_doc["url"] = "https://api.nhs.uk".concat(
                    relatedLink[i]["url"].toString()
                  );
                } else {
              ...
              for (let i = 0; i < mainEntity_len; i++) {
                if (mainEntity[i].headline === undefined) {
                  switch (mainEntity[i]["@type"]) {
                    ...
                    case "WebPageElement":
                      if (
                        mainEntity[i].name !== "markdown" &&
                        mainEntity[i].name !== "Reveal"
                      ) {
                    ...
                    case "Question":
                      ...
                      let answer =
                        mainEntity[i]["acceptedAnswer"]["mainEntity"][0][
                          "text"
                        ];
                      DocumentContent.text.push("Answer");
                      DocumentContent.text.push(answer);
                  ...
                  if (
                    mainEntity[i].headline !== "" &&
                    mainEntity[i].headline !== null
                  ) {
                    DocumentContent.text.push(mainEntity[i].headline);
                  }
                  if (
                    mainEntity[i].text !== "" &&
                    mainEntity[i].text !== null
                  ) {
                    DocumentContent.text.push(mainEntity[i].text);
                  }
                  await allText(
                    mainEntity[i].mainEntityOfPage,
                    DocumentContent
                  );
                }
              }
            ...
            let allLinks = async (mainEntity) => {
              ...
              for (let i = 0; i < mainEntity_len; i++) {
                if (mainEntity[i].url !== undefined) {
                  ...
                  mainEntity[i].mainEntityOfPage["hasPart"] !== undefined
                ) {
